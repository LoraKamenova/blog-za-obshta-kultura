...
class Baking extends Component {
    ...
    render() {
        ...
        let recipes = reversedData.filter(temp => temp.subcategory.includes("Тестени")).map(({_id, title, url, subcategory, content, date}) => ({_id, title, url, subcategory, content, date}));
    
        return (
            <section className="all-shared-page">
                <h1 className='all-shared-heading'>Категория "Тестени"</h1>
