...
class Sofia extends Component {
    ...
    render() {
        ...
        let books = reversedData.filter(temp => temp.subcategory.includes("Книги за София")).map(({_id, title, url, subcategory, content, date}) => ({_id, title, url, subcategory, content, date}));
    
        return (
            <section className="all-shared-page">
                <h1 className='all-shared-heading'>Категория "Книги за София"</h1>
